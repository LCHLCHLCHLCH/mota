...
| ID | 说明 |
|---|---|
| 136 | 怪物身躯（多格 Boss 的身体，不可通行；触碰任意格触发对首领的战斗） |
| 255 | 勇者（玩家）——不要在地图数据中使用 |

> **多格 Boss**：15 层巨型章鱼、35 层魔龙用 3×3 的 `怪物身躯`(136) 表示。引擎会在地图上叠加一个覆盖整块的大号首领字符，并在块右下角画一个威胁指示灯。对应事件用 `trigger = "on_tile", tile = 136` + `battle_monster(首领 id)`：胜利后清除整个身躯块并移动玩家。
